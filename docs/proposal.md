...
- 参赛者：（待填写）
- 联系方式：（待填写）
- GitHub 仓库链接：https://github.com/PaiGack/moonbit_ftp
...
3. **CI / 部署脚本**：构建产物通过 `stor` 上传到运维 FTP，`mfmt`/`mdtm` 校准文件时间戳，
...
移植的具体执行步骤（工作包拆解、上游落点映射、测试策略、兼容性清单、验收标准）
见 [porting/](./porting/) 文档集，入口：[porting/README.md](./porting/README.md)。
